...
import com.hf.live.util.FyjpCustomMultipartEntity.ProgressListener;
...
public class FyjpHttpMultipartPost extends AsyncTask<String, Integer, String> {
	...
	public FyjpHttpMultipartPost(Context context, String url, AsynLoadCompleteListener completeListener) {
	...
	private void showDialog() {
		if (mDialog == null) {
			mDialog = new FyjpUploadDialog(context);
	...
	protected String doInBackground(String... params) {
		...
		try {
			FyjpCustomMultipartEntity multipartContent = new FyjpCustomMultipartEntity(new ProgressListener() {
						@Override
			...
			multipartContent.addPart("appid", new StringBody(FyjpCONST.APPID, Charset.forName("UTF-8")));
